...
class Accuracy(Metric):
    ...
    def compute(self, y_true: list[int], y_pred: list[int]) -> float:
        ...
        # TODO: 정확도를 계산하세요
        # 힌트:
        # 1) 빈 리스트인 경우 0.0 반환 ; (y_true == None ->len(y_true)==0 ->ZeroDividionError)예방 및, 불필요 연산 최소화.
        # 2) correct = sum(1 for t, p in zip(y_true, y_pred) if t == p) ; 정답label과 예측을 iterate한 tuple 짝으로 비교해서 correct prediction의 개수 집계. ->int
        # 3) return correct / len(y_true) -> float
